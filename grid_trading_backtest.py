...
def fetch_stock_data(symbol: str, start_date: str, end_date: str, interval: str = '5m') -> pd.DataFrame:
    ...
    try:
        ...
        ak_period = period_map.get(interval, '5')
...
class GridTradingEngine:
    ...
    def __init__(
        ...
        min_shares: int = 100,        # 最少持股数
        max_shares: int = 10000,      # 最大持股数
    ...
    def run(self):
        ...
        for i, row in self.data.iterrows():
            ...
            if price_change_pct >= self.sell_pct:
                # 触发卖出
                qty = min(self.grid_lot, self.shares - self.min_shares)
                if qty > 0 and self.shares - qty >= self.min_shares:
    ...
    def _calc_buy_qty(self, price: float) -> int:
        ...
        # 检查最大持股限制
        if self.shares + qty > self.max_shares:
    ...
    def run_backtest(event):
        ...
        try:
            ...
            engine = GridTradingEngine(
                ...
                min_shares=min_shares_input.value,
                max_shares=max_shares_input.value,
            ...
            # ---- 更新图表 ----
            kline = create_kline_chart(data, trades_df, symbol)
            ...
            if not trades_df.empty:
                ...
                trades_table_pane.append(
                    pn.pane.Markdown(f"## 📝 交易记录（共 {len(trades_df)} 笔）")
                )
                ...
                trades_table_pane.append(
                    pn.pane.Alert("未产生任何交易记录，请调整网格参数", alert_type='warning')
                )
...
if __name__ == '__main__':
    ...
    pn.serve(app, port=5006, show=True, title='网格交易回测')